...
class ThreatAnalyzer:
    VIOLENCE_KEYWORDS = {
        ...
        "rape": 95, "stab": 85, "threaten": 70, "hurt": 65,
        "destroy": 70, "revenge": 75, "eliminate": 80, "gun": 60,
        "knife": 55, "weapon": 65, "arsenal": 75,
    }
    
    THREAT_CATEGORIES = {
        "physical_violence": ["kill", "murder", "shoot", "attack", "stab"],
        "terrorism": ["terrorist", "bomb", "explosion", "massacre"],
        "self_harm": ["suicide", "want to die"],
        "harassment": ["threaten", "harass", "stalk"],
        "property_crime": ["steal", "rob", "burglary"],
    }
...
def handler(request):
    ...
    if path == "/prediction":
        high_risk = by_level = {"critical": 0, "high": 0}
        for t in threat_log:
